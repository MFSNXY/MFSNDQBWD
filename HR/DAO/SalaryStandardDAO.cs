...
namespace DAO
{
    public class SalaryStandardDAO : BaseDao<SalaryStandard>, ISalaryStandardDAO
    {
        ...
        {
            string sql = string.Format("select * from dbo.SalaryStandard where 1=1");
            if (xcid != null && xcid != "")
            {
            ...
            if (cs != null && cs != "")
            {
                sql += string.Format(" and Checkstatus='{0}'", cs);
            }
            if (gjz != null && gjz != "")
            {
        ...
        {
            MyDbContext db = CreateContext();
            string sql = string.Format(@"update dbo.SalaryStandard set Changer='{0}',Changetime='{1}',Remark='{2}',Checkstatus=0,Salarysum={3} where Id ={4}", ck.Changer, ck.Changetime, ck.Remark, ck.Salarysum, ck.Id);
